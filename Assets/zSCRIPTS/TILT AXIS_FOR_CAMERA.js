// Hacked and gutted from the MouseOrbit script that comes with 
...
function FixedUpdate () {
	...
	{
		GameObject.FindWithTag("MainCamera").rigidbody.AddRelativeForce(0, 0, normalizedSpeed * (forwardForce*3));
		...
		euler.z = Mathf.Lerp(euler.x, -accelerator.y * maxTurnLean, 0.2);

		// Since we set absolute lean position, do some extra smoothing on it
		euler.z = Mathf.Lerp(euler.x, accelerator.y * maxTilt, 0.2);
	
		// Apply rotation and apply some smoothing
		var rot : Quaternion = Quaternion.Euler(euler);
		
		GameObject.FindWithTag("MainCamera").transform.rotation = Quaternion.Lerp (transform.rotation, rot, sensitivity);
	}}
